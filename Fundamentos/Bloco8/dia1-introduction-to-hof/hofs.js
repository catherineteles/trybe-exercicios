...
const emailInformation = (fullname) => {
    //https://www.w3schools.com/jsref/jsref_join.asp
    const email = fullname.split(' ').join('_');
    return {fullname, email: `${email.toLowerCase()}@trybe.com` }
}
const newEmployees = (callback) => {
  ...
  console.log(newEmployees(emailInformation));
...
const lottery = (num, callback) => {
    const lotteryNumber = Math.round(Math.random() * 5);
    if( callback(num,lotteryNumber)){
        return "Parabéns você ganhou"
    }
    return `Tente novamente, o numero sorteado foi ${lotteryNumber}`
};
